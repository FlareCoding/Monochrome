...
public:
    ...
    void addRowDefinition(Shared<RowDefinition> rowDef = MakeRef<RowDefinition>());
    void addColumnDefinition(Shared<ColumnDefinition> colDef = MakeRef<ColumnDefinition>());
    ...
    // Triggers a layout update in case any row or column definition changed
    void updateLayout();
...
protected:
    ...
    uint32_t _calculateRowHeight(size_t rowIdx);
    uint32_t _calculateColumnWidth(size_t colIdx);
